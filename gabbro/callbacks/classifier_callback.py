...
class ClassifierEvaluationCallback(L.Callback):
    ...
    def on_test_epoch_end(self, trainer, pl_module):
        ...
        if hasattr(pl_module, "head_class"):  # new multi-head model
            if pl_module.head_class is None:
                self.logger.warning(
                    "No classification head found. Skipping saving test predictions."
                )
                return {}
            results_akarr = ak.Array(
                {
                    "test_class_preds": np.concatenate(pl_module.test_class_preds_list),
                    "test_class_targets": np.concatenate(pl_module.test_class_target_list),
                }
            )
        else:
            results_akarr = ak.Array(
        ...
        self.logger.info(results_metrics_dict)
    ...
    def compare_val_test_vs_train(self, trainer, pl_module, stage="val"):
        ...
        if stage == "val":
            if hasattr(pl_module, "head_class"):  # new multi-head model
                if (
                    len(pl_module.val_class_target_list) == 0
                    or len(pl_module.train_class_target_list) == 0
                ):
                    self.logger.info("No classification targets found. Skipping plotting.")
                    return
            elif not hasattr(pl_module, "val_preds_list") or not hasattr(
                pl_module, "val_labels_list"
            ):
                self.logger.info("No validation predictions found. Skipping plotting.")
                return
            elif not hasattr(pl_module, "train_preds_list") or not hasattr(
            ...
            if hasattr(pl_module, "head_class"):  # new multi-head model
                if len(pl_module.test_class_target_list) == 0:
                    self.logger.info("No classification targets found. Skipping plotting.")
                    return
            elif not hasattr(pl_module, "test_preds_list") or not hasattr(
        ...
        if stage == "val":
            if hasattr(pl_module, "head_class"):  # new multi-head model
                n_classes = pl_module.val_class_target_list[0].shape[1]
                val_labels = np.concatenate(pl_module.val_class_target_list)
                val_preds = np.concatenate(pl_module.val_class_preds_list)
                train_labels = np.concatenate(pl_module.train_class_target_list)
                train_preds = np.concatenate(pl_module.train_class_preds_list)
            else:  # old model
                n_classes = pl_module.val_labels_list[0].shape[1]
                val_labels = np.concatenate(pl_module.val_labels_list)
                val_preds = np.concatenate(pl_module.val_preds_list)
                train_labels = np.concatenate(pl_module.train_labels_list)
                train_preds = np.concatenate(pl_module.train_preds_list)
        elif stage == "test":
            if hasattr(pl_module, "head_class"):  # new multi-head model
                n_classes = pl_module.test_class_target_list[0].shape[1]
                test_labels = np.concatenate(pl_module.test_class_target_list)
                test_preds = np.concatenate(pl_module.test_class_preds_list)
            else:  # old model
        ...
        if stage == "val":
            ...
            # check that all lists are non-empty (this helps to be more robust against
            # checkpoint loading runs)
            if (
                len(val_preds) == 0
                or len(val_labels) == 0
                or len(train_preds) == 0
                or len(train_labels) == 0
            ):
                self.logger.info("No predictions found. Skipping plotting.")
                return None
            # check that all lists have the same length
            if len(val_preds) != len(val_labels) or len(train_preds) != len(train_labels):
                self.logger.info(
                    "Prediction and label arrays have different lengths. Skipping plotting."
                )
                return None
            val_is_bkg = val_labels[:, 0] == 1
            val_scoreB = val_preds[:, 0]
            train_is_bkg = train_labels[:, 0] == 1
            train_scoreB = train_preds[:, 0]
            ...
            val_acc_multiclass = calc_accuracy(val_preds, val_labels)
            train_acc_multiclass = calc_accuracy(train_preds, train_labels)
            ...
            val_is_bkg = test_labels[:, 0] == 1
            val_scoreB = test_preds[:, 0]
            val_acc_multiclass = calc_accuracy(test_preds, test_labels)
            results_dict_all_classes["multiclass_accuracy_test"] = float(val_acc_multiclass)

        for sig_class in range(1, n_classes):
            if stage == "val":
                val_is_signal = val_labels[:, sig_class] == 1
                train_is_signal = train_labels[:, sig_class] == 1
                ...
                val_is_signal = test_labels[:, sig_class] == 1
    ...
    def plot_classifier_output(
        ...
        if trainer.global_rank == 0:
            if self.log_images:
                ...
                self.logger.info("Not saving/logging plot since `log_images` is set to False.")
        plt.close()
        results_dict = {
