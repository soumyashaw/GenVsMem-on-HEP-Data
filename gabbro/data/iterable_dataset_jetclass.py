...
class CustomIterableDataset(IterableDataset):
    ...
    def __init__(
        ...
        **kwargs
        ...
        valid_dataset_types = ["jetclass", "aoj", "lhco"]
        ...
        if self.token_reco_cfg is not None:
            class_token_dict = self.token_id_cfg.get("class_token_dict", None)
        else:
            class_token_dict = None

        if class_token_dict is not None:
            # Check that the class token dictionary makes sense. Tokens must be integers from 1 to n_classes.
            tokens = list(class_token_dict.values())
            tokens.sort()
            n_classes = len(class_token_dict)
            allowed_tokens = list(range(1, n_classes + 1, 1))
            assert tokens == allowed_tokens, (
                "Error in provided class_token_dict, check that the tokens range from 1 to n_classes."
            )
            # Accommodate class token
            self.pad_length = self.pad_length + 1
    ...
    def load_next_files(self):
        ...
        for i_file, filename in enumerate(self.current_files):
            ...
            if self.token_reco_cfg is not None:
                # This mean that a tokenized file is loaded and reconstructed
                # to the initial data space
                _, ak_x_particles, ak_jet_type_labels = reconstruct_jetclass_file(
                    ...
                    device=self.multi_gpu_info["device"],
                    return_labels=True,
                )
                self.logger.info("Calculating additional kinematic features.")
                ak_x_particles = combine_ak_arrays(
                    calc_additional_kinematic_features(
                        ak_x_particles,
                        exclude_fields=ak_x_particles.fields,
                    ),
                    ak_x_particles,
                )
                ...
                if self.token_id_cfg.get("feature_resolution", None) is not None:
                    ...
                    if self.token_id_cfg["feature_resolution"] not in allowed_feature_resolutions:
                        raise ValueError(
                ...
                if particle_features_tokenized is not None or particle_features is not None:
                    self.logger.info("Calculating additional kinematic features.")
                    ak_x_particles = combine_ak_arrays(
                        calc_additional_kinematic_features(
                            ak_x_particles,
                ...
                if self.dataset_type == "lhco":
                    ...
                    ak_x_particles, ak_jet_type_labels = load_lhco_jets_from_parquet(
                        filename,
                        feature_dict=pp_dict_with_spectators,
                        n_jets=None,  # all
                    )
                    ak_x_jet = None
                elif self.dataset_type == "aoj":
            ...
            if self.n_jets_per_file is not None:
                self.logger.info(f"Selecting the first {self.n_jets_per_file} jets from the file.")
            ...
            if self.feature_dict_jet is not None:
                ...
                self.jet_data_list.append(torch.tensor(np_x_jet))
            ...
            if self.feature_dict_labels_particles is not None:
                ...
                self.labels_particles_data_list.append(torch.tensor(np_labels_particles))

            # add the data to the lists
            if not self.collate:
                self.part_data_list.append(
                ...
                self.part_data_list.append(ak_x_particles)  # Note that we still have ak here
