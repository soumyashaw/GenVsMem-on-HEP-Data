...
def tokenize_jetclass_file(
    ...
    binning_cfg: str = None,
    save_token_id_only: bool = False,
    dataset_type: str = "jetclass",
    shuffle_particles: bool = False,
):
    """Tokenize a single file using a trained VQ-VAE model or binning.
    ...
    binning_cfg : str, optional
        Path to the binning configuration file.
    save_token_id_only : bool, optional
        Whether to save only the token ids.
    dataset_type : str, optional
        The type of dataset. Options are "jetclass", "landscape", "cms_open_data", or "lhco". Default is "jetclass".
    shuffle_particles : bool, optional
        Whether to shuffle the particles. Default is False.
    """

    # input verification
    if model_ckpt_path is None and binning_cfg is None:
        raise ValueError("Either `model_ckpt_path` or `binning_cfg` must be provided.")
    elif model_ckpt_path is not None and binning_cfg is not None:
        raise ValueError("Only one of `model_ckpt_path` or `binning_cfg` can be provided.")
    elif model_ckpt_path is not None:
        logger.info(f"Using model checkpoint: {model_ckpt_path}")
    elif binning_cfg is not None:
        logger.info(f"Using binning configuration: {binning_cfg}")

    # Validate dataset_type
    valid_dataset_types = ["jetclass", "landscape", "cms_open_data", "lhco"]
    ...
    if model_ckpt_path is not None:
        ...
        cfg = OmegaConf.load(binning_cfg)
        binning_dict = {
            key: np.linspace(*cfg.particle_features[key]["binning"])
            for key in cfg.particle_features
        }
        pp_dict = {key: cfg.particle_features[key]["pp_params"] for key in cfg.particle_features}
        pp_dict_jet = (
            {key: cfg.jet_features[key]["pp_params"] for key in cfg.jet_features}
            if "jet_features" in cfg
            else {}
        )
    ...
    if dataset_type == "cms_open_data":
        ...
        x_ak, _ = load_lhco_jets_from_parquet(
            filename_in,
            feature_dict=pp_dict_with_spectators,
            n_jets=None,  # all
        )
        x_ak_jet = None
    else:
    ...
    if model_ckpt_path is not None:
        ...
        #  - part_token_id
        #    | part_token_id_group_0
        #    | part_token_id_group_1
        #    | ...
        #  - z_q
        #    | z_q_0
        #    | z_q_1
        ...
        tokenized_features = tokenizer.tokenize_ak_array(
            ak_arr=x_ak,
            pp_dict=pp_dict,
        )
        token_ids = tokenized_features["part_token_id"]  # equivalent to group quantization
        tmp_key = (
            "part_features_tokenized"
            if "part_features_tokenized" in tokenized_features.fields
            else "part_features"
        )
        features_tokenized = ak_select_and_preprocess(
            ak_array=tokenized_features[tmp_key],
            pp_dict=pp_dict,
            inverse=True,
        )
    ...
    if add_start_end_tokens:
        if model_ckpt_path is not None:
            ...
            # check that all features are tokenized with the same number of bins
            # (current limitation of the setup)
            bin_sizes = tokenizer.bin_sizes
            bin_sizes_unique = np.unique(np.array(bin_sizes))
            if len(bin_sizes_unique) > 1:
                raise ValueError(
                    "With the current setup, all features need to be tokenized with "
                    f"the same number of bins. Provided bin sizes: {bin_sizes}"
                )
            n_tokens = bin_sizes_unique[0]
        token_ids_plus_one = (
            token_ids + 1
...
def reconstruct_jetclass_file(
    ...
    tokenizer_type : str, optional
        Type of the tokenizer used. Either "vqvae" or "binning".
    ...
    if tokenizer_type == "vqvae":
        ...
        #     logger.info(f" | {key}: {value}")
    elif tokenizer_type == "binning":
        cfg = OmegaConf.load(config_path)
        pp_dict = cfg.particle_features
        pp_dict = {key: pp_dict[key]["pp_params"] for key in pp_dict}
        model = torch.load(model_ckpt_path)  # nosec
        pp_dict_jet = cfg.jet_features if "jet_features" in cfg else {}
    else:
        supported_tokenizer_types = ["vqvae", "binning"]
        raise ValueError(
    ...
    # for one-dimensional tokens, the expected structure is
    #  - part_token_id
    #    | part_token_id
    # for group quantization, the expected structure is
    #  - part_token_id
    #    | part_token_id_group_0
    #    | part_token_id_group_1
    #    | ...
    ...
    if len(tokenized_file_ak_arr.fields) > 0:
        ...
        if len(tokens.fields) > 1:
            logger.info("Group quantization detected")
            logger.info(f"Number of token groups: {len(tokens.fields)}")
            n_token_groups = len(tokens.fields)
        else:
            n_token_groups = 1
            if len(tokens.fields) == 1:
    ...
    if n_token_groups > 1:
        # group quantization
        logger.info(f"Group quantization with {n_token_groups} groups")
        ak_arr_dict_tmp = {}
        for field in tokens.fields:
            ak_arr_dict_tmp[field] = tokens[field]
        if start_token_included:
            logger.info("Removing start token")
            for field in tokens.fields:
                ak_arr_dict_tmp[field] = ak_arr_dict_tmp[field][:, 1:]
        ...
        if shift_tokens_by_minus_one:
            ...
            for field in tokens.fields:
                ak_arr_dict_tmp[field] = ak_arr_dict_tmp[field] - 1
        for k, v in ak_arr_dict_tmp.items():
            logger.info(f"{k}: min={ak.min(v)}, max={ak.max(v)}")
        tokens = ak.Array(ak_arr_dict_tmp)
