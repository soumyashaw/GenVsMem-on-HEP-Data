...
def shuffle_ak_arr_along_axis1(ak_arr, seed=42):
...
def tensor_px(pt, phi):
    """
    Calculates px for tensors using torch methods only.
    ...
    torch.tensor
    """
    assert torch.is_tensor(pt) and torch.is_tensor(phi), (
        f"pt and phi must be tensors, got pt {type(pt)} and phi {type(phi)}."
    )
    px = pt * torch.cos(phi)
    return px


def tensor_py(pt, phi):
    """
    Calculates py for tensors using torch methods only.
...
def tensor_pz(pt, eta):
    """
    Calculates pz for tensors using torch methods only. Assumes massless particles.
    ...
    torch.tensor
    """
    assert torch.is_tensor(pt) and torch.is_tensor(eta), (
        f"pt and eta must be tensors, got pt {type(pt)} and eta {type(eta)}."
    )
...
def tensor_E(pt, eta):
    """
    Calculates E for tensors using torch methods only. Assumes massless particles.
...
def jetmass_from_reconstructed_tokens(reconstructed_constituents):
    """
    Parameters
    ----------
    reconstructed_constituents: torch.tensor
        Should have shape (N_events, max_seq_len + 1, 3), where the last
        dimension should have order pt, eta, phi.
    """

    if not len(reconstructed_constituents.shape) == 3:
        raise ValueError(
            f"Expected input tensor to have 3 dimensions, but got {len(reconstructed_constituents.shape)}."
        )
    if not reconstructed_constituents.shape[-1] == 3:
        raise ValueError(
            f"Expected each constituent to have 3 features, but got {reconstructed_constituents.shape[-1]}."
        )

    reconstructed_constituents = reconstructed_constituents.to(torch.float64)

    pt = reconstructed_constituents[:, :, 0]
    assert (reconstructed_constituents[:, :, 0] >= 0).all(), (
        "Negative pT values, cannot calculate jet mass"
    )

    eta = reconstructed_constituents[:, :, 1]
    phi = reconstructed_constituents[:, :, 2]

    px = tensor_px(pt, phi)
    py = tensor_py(pt, phi)
    pz = tensor_pz(pt, eta)
    E = tensor_E(pt, eta)
    assert (E >= 0).all(), "Negative energy obtained, aborting jet mass calculation"

    p4 = torch.zeros(reconstructed_constituents.shape[0], reconstructed_constituents.shape[1], 4)
    ...
    jet_p4 = p4.sum(dim=1)  # Sum over constituents, shape (N_events, 4)
    m2 = jet_p4[:, 0] ** 2 - (jet_p4[:, 1] ** 2 + jet_p4[:, 2] ** 2 + jet_p4[:, 3] ** 2)

    number_of_negative_masses = 0

    if not (m2 >= 0).all():
        negative_mask = m2 < 0
        number_of_negative_masses = negative_mask.sum()
        m2[negative_mask] = 0

    assert (m2 >= 0).all(), "m^2 is negative, aborting"

    jet_mass = torch.sqrt(m2)  # Shape (N_events)

    return jet_mass, number_of_negative_masses


def set_fraction_ones_to_zeros(mask: torch.Tensor, fraction: torch.Tensor):
    """Set a fraction of the ones in a mask to zero. The mask is modified in place.
