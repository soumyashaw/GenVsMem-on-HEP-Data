...
class NormformerBlock(nn.Module):
    def __init__(
        ...
        self.mlp = nn.Sequential(
            ...
            # nn.LayerNorm(self.input_dim * self.mlp_expansion_factor),  # TODO: add this, but didn't so so in the original implementation, so keep if for now
...
class Residual(nn.Module):
    ...
    def __init__(self, module, gate_type="global", init_value=1.0):
        """
        Parameters
        ----------
        module: nn.Module
...
class EncoderBlock(nn.Module):
    ...
    def __init__(self, attn_cfg=None, mlp_cfg=None, residual_cfg=None):
        super().__init__()
        self.attn = Residual(AttentionBlock(**attn_cfg), **residual_cfg)
