...
from vqtorch.nn import GroupVectorQuant, VectorQuant
...
class VQVAETransformer(torch.nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        vq_kwargs = dict(vq_kwargs or {})
        # NOTE: These lines are for backwards compatibility with trained
        # tokenizer VeraciousCub_12253053--------------------------------------
        max_sequence_len_override = vq_kwargs.pop("max_sequence_len", None)
        vq_kwargs.setdefault("dim", -1)
        self.vq_kwargs = dict(vq_kwargs)
        self.input_features_dict = input_features_dict
        if input_features_dict is not None:
        ...
        if max_sequence_len_override is not None:
            self.max_sequence_len = max_sequence_len_override
        ...
        mlp_cfg = {"expansion_factor": 4, "dropout_rate": 0.0, "norm_before": True}
        ...
        vq_kwargs_layer = dict(vq_kwargs)

        if vq_kwargs_layer.get("groups", 1) > 1:
            self.vqlayer = GroupVectorQuant(feature_size=self.latent_dim, **vq_kwargs_layer)
        else:
            # drop the groups key if it's in there but equals 1
            vq_kwargs_layer.pop("groups", None)
            # same for `share`
            vq_kwargs_layer.pop("share", None)
            self.vqlayer = VectorQuant(feature_size=self.latent_dim, **vq_kwargs_layer)
        ...
        if not self.old_transformer_implementation:
            ...
            # for backwards compatibility with the old transformer implementation
            if self.causal_decoder:
                self.decoder_normformer = GPT_DecoderStack(
                    embedding_dim=self.hidden_dim,
                    attention_dropout=0.0,
                    n_heads=self.num_heads,
                    apply_causal_mask=True,
                    max_sequence_len=self.max_sequence_len,
                    n_GPT_blocks=self.num_blocks,
                )
            else:
...
class VQVAELightning(L.LightningModule):
    ...
    def tokenize_ak_array(
        ...
        # check if we are using group vector quantization (if yes, there is
        # an additional dimension in the codes)
        if isinstance(self.model.vqlayer, GroupVectorQuant):
            # get the number of groups
            n_groups = self.model.vqlayer.groups

            if not self.model.vqlayer.share:
                raise ValueError("GroupVectorQuant with share=False not supported.")

            print(f"Using GroupVectorQuant with {n_groups} groups.")
            feature_names = [f"part_token_id_group_{i}" for i in range(n_groups)]

        elif isinstance(self.model.vqlayer, VectorQuant):
        ...
        with torch.no_grad():
            ...
            for i, batch in enumerate(pbar):
                ...
                try:
                    ...
                    if isinstance(self.model.vqlayer, GroupVectorQuant):
                        z_q = self.apply_group_embedding(tokens_batch)
                    else:
    ...
    def apply_group_embedding(self, q):
        # q are codes of shape (B, S, G) where G is the number of groups
        # and S is the sequence length

        # check embedding dimension (per group)
        embed_dim = self.model.vqlayer.codebook.weight.shape[1]
        n_groups = self.model.vqlayer.groups
        # initialize tensor for embeddings (after applying the codebook)
        z_q = torch.zeros(q.shape[0], q.shape[1], n_groups, embed_dim).to(q.device)

        for i in range(self.model.vqlayer.groups):
            # get codebook for group i
            cb, offset = self.model.vqlayer.get_codebook_by_group(i)
            # apply affine transform if present
            if hasattr(self.model.vqlayer, "affine_transform"):
                cb = self.model.vqlayer.affine_transform(cb)
            # assign to tensor
            z_q[..., i : i + 1, :] = F.embedding(q[..., i : i + 1], cb)

        # stack them along the last dimension
        z_q = z_q.reshape(q.shape[0], q.shape[1], -1)
...
def plot_loss(loss_history, lr_history, moving_average=100):
    if len(loss_history) < moving_average:
        logger.info("Not enough steps to plot loss history")
        return
