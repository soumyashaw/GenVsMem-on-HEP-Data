...
class BackboneMultiHeadLightning(L.LightningModule):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.hparams.class_head_cfg is None:
            ...
            self.head_class = ClassifierNormformer(
                input_dim=backbone_cfg["embedding_dim"],
                hidden_dim=128,
                num_heads=8,
                num_class_blocks=2,
            ...
            #     input_dim=backbone_cfg["embedding_dim"],
            #     intermediate_dim=self.hparams.class_head_cfg.get("intermediate_dim", 512),
            #     output_dim=self.hparams.class_head_cfg.get("n_out_nodes", 2),
            # )
        ...
        # --- FOR DEBUGGING:
        # set the masked feature vector to ones * index for easy identification
        # if the positional encoding works as expected
        # self.masked_feature_vector.data = torch.ones_like(
        #     self.masked_feature_vector.data
        # )
        # for i in range(self.masked_feature_vector.shape[0]):
        #     self.masked_feature_vector.data[i, :] = i*10
        ...
        if self.hparams.start_token_type in ["trainable", "zero_pad"]:
            ...
            if (
                "token_id" in self.backbone.embed_cfg.type
        ...
        if self.hparams.masked_input_treatment == "pad_trainable":
            raise ValueError(
                "The masked input treatment 'pad_trainable' is not "
                "supported in the current implementation. Please use 'exclude' "
                "or 'zero_pad' instead."
            )
            # if the masked input treatment is "pad_trainable", we want to
            # make the start token a trainable parameter
            self.input_padding_vector = nn.Parameter(
                torch.zeros(self.backbone.part_features_input_dim)
            )
        ...
        if backbone_cfg.get("mask_fraction", 0.0) == 0.0:
            self.pylogger.info(
                "Mask fraction is 0, detaching the masked feature vector from the gradient computation."
            )
    ...
    def forward(
        ...
        if valid_particle_mask is None:
            ...
            mask_for_backbone_input = valid_particle_mask
        elif self.hparams.masked_input_treatment == "pad_trainable":
            # set the masked particles to the trainable padding vector
            x[valid_particle_but_masked_mask == 1] = self.input_padding_vector
            mask_for_backbone_input = valid_particle_mask
        ...
        if self.hparams.pos_encoding_type is None:
            # -- no positional encoding (as long as particles are shuffled) --
            backbone_out[indices] = self.masked_feature_vector[indices[1]]
        elif (
            self.hparams.pos_encoding_type == "sort_by_first_feature_descending"
            or self.hparams.pos_encoding_type
            == "sort_by_first_feature_descending_in_masked_subset"
        ):
            # calculate sorting of particles (first feature in the continuous input)

            # clone sorting feature cause we'll modify this tensor in place
            proxy_sort_feat = x[:, :, 0].clone()

            # set invalid particles to -inf (0-padded values otherwise don't
            # end up at the end of the sequence) --> with this they are sorted to the end
            proxy_sort_feat[valid_particle_mask == 0] = float("-inf")  # set invalid particles

            if (
                self.hparams.pos_encoding_type
                == "sort_by_first_feature_descending_in_masked_subset"
            ):
                # set particles that survived the masking to -inf
                # --> by doing this, the only non-minus-inf values are the masked particles
                proxy_sort_feat[valid_particle_mask_corrupted == 1] = float("-inf")

            sort_index = torch.argsort(proxy_sort_feat, dim=1, descending=True)
            rank_index = torch.argsort(sort_index, dim=1)
            # # --- for inspecting the sorting ---
            # print(f"proxy_pt[0]: {proxy_sort_feat[0]}")
            # print(f"pt_sort_index[0]: {sort_index[0]}")
            # print(f"rank_index[0]: {rank_index[0]}")

            # use the rank indices to get the masked feature vector
            backbone_out[indices] = self.masked_feature_vector[
                rank_index[indices[0], indices[1]], :
            ]

        # --- for inspecting the positional encoding ---
        # - print the backbone output (first dim) of the first jet
        # - print the mask which tells which particles survived the masking
        # print(f"Backbone out (first feat dim) of the first jet: {backbone_out[0, :60, 0]}")
        # print(f"Valid particle mask corrupted: {valid_particle_mask_corrupted[0, :60]}")
    ...
    def model_step(self, batch):
        ...
        if self.head_mpm is not None:
            if self.hparams.exclude_padded_values_from_loss:
                # if old head definition is used, the logits are already fixed
                # --> unsqueeze last dim
        ...
        if self.head_gen is not None:
            ...
            # TODO: this is basically already done above, so remove this at some point
            # and always exclude padded values from loss (depending on which mask is used)
            if self.hparams.exclude_padded_values_from_loss:
                targets_gen = targets_gen * mask_for_gen_loss
    ...
    def _collect_batch_data(self, batch, model_step_output, stage: str) -> None:
        ...
        if self.head_mpm is not None:
            ...
            mpm_pred_list.append(
                torch.argmax(model_step_output["logits_mpm"], dim=-1)
                .float()
                .detach()
                .cpu()
                .numpy()
    ...
    def generate_batch_continuous(
        ...
        for i in range(self.max_sequence_len):
            ...
            if "token_id" in self.backbone.embed_cfg.type:
    ...
    def convert_valtest_batches_to_ak(self, stage):
        ...
        if stage == "val":
            ...
            target_gen = (
                np.concatenate(self.val_gen_target_list)
                if len(self.val_gen_target_list) > 0
                else None
            )
            ...
            target_gen = (
                np.concatenate(self.test_gen_target_list)
                if len(self.test_gen_target_list) > 0
                else None
            )
